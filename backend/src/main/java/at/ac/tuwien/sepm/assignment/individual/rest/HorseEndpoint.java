...
public class HorseEndpoint {
  ...
  public Stream<HorseListDto> searchHorses(HorseSearchDto searchParameters) {
    LOG.info("GET " + BASE_PATH + " query parameters: {}", searchParameters.maxAmount());
  ...
  public HorseDetailDto post(@RequestBody HorseCreateDto toCreate) throws ValidationException, ConflictException {
  ...
  @GetMapping(path = "{id}/familytree/{generation}")
  public HorseFamilyTreeDto getFamilyTree(@PathVariable("id") long id, @PathVariable("generation") int generation) {
    LOG.info("GET " + BASE_PATH + "/{}/familytree/{}", id, generation);

    try {
      return service.getFamilyTree(id, generation);
    } catch (NotFoundException e) {
      HttpStatus status = HttpStatus.NOT_FOUND;
      logClientError(status, "Horse whose family tree is fetched is not found", e);
      throw new ResponseStatusException(status, e.getMessage(), e);
    }
  }
